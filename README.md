...
1. **트랜지스터 (스위치)**: 반도체 성질을 이용해 전기를 켜고 끄며 0과 1의 신호를 만듭니다.
2. **논리 회로 (계산기)**: 스위치들을 조합해 논리 게이트(AND, OR 등)를 만들고, 이를 통해 이진수 덧셈 같은 연산을 수행합니다.
3. **저장 장치**: CPU 내부에는 가장 빠른 메모리인 레지스터와 속도 차이를 줄여주는 캐시 메모리가 있어 데이터를 효율적으로 주고받습니다.
4. **작동 순서**: 메모리에서 명령을 가져오고(인출) → 분석해서(해석) → 계산하고(실행) → 저장하는 과정을 무한 반복합니다.
...
이후 렉처들은 보면서 동작하는 코드가 나오면 멈추고 타이핑하고 실행해보자.

각 Lecture 들은 약 2시간씩임. Lecture 하나에 몇시간(며칠)이 걸리는지 확인 필요.
...
Chap1 그냥 듣는 것으로 충분.
(알고리즘은 레시피다, 다른 부분은 bRd 3D 의 설명과 겹치는 부분이 있다, 이후에 프로그래밍 언어에 대해)
...
- 한빛 https://www.youtube.com/watch?v=Oga3_Rl1kxQ&list=PLVsNizTWUw7FCS83JhC1vflK8OcLRG0Hl&index=4
- 버클리 https://www.youtube.com/playlist?list=PLhMnuBfGeCDM8pXLpqib90mDFJI-e1lpk
- CMU https://www.youtube.com/playlist?list=PLtv4PQFH2yMas_SCMTEaE4eUIWWWiwISj
  - https://www.cs.cmu.edu/~18213/lectures/
- MIT https://ocw.mit.edu/courses/6-004-computation-structures-spring-2017/resources/concrete-encoding-of-information-4-16-

프로그래밍 쪽은 CMU가 더좋고, 하드웨어 쪽 (캐시파이프라인)은 버클리가 더 좋은듯. MIT도 버클리비슷(컴파일러, 어셈은 나옴)

### NAND to Tetris

이건 컴구조를 이해할 수 있는 독특한 강의
...
- https://www.youtube.com/playlist?list=PL9k2wIz8VsfMTEIu_Fy9C_shF0dUzNT6h
...
- https://www.youtube.com/playlist?list=PLzBbfbHQmjyuqBFJ8KpDdcvnLNkvPXbS-
...
### CS231n 
Deep Learning for computer vision. Stanford, by Fei-Fei Li et al.
...
- https://x.com/Vinay_bharambe/status/1989347727524995459
- https://x.com/ChShersh/status/1989066805667852571
- https://x.com/Loop_and_lift/status/1973959191636201786
